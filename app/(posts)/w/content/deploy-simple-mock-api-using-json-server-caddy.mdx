...
```caddyfile showLineNumbers
your-site.com {
  reverse_proxy node:5005
}
```

Lưu ý vì dùng `localhost:5005` thì chúng ta phải lấy tên của container được setup trong `docker-compose`, trong trường hợp này mình đặt tên là `node`. Như vậy, mỗi khi mình truy cập vào `your-site.com`, thì Caddy sẽ điều hướng tới tới mock API đang chạy.
